...
"""
def get_all_data_for_charts(groupName, rfid, nDays):
	...
	for i in range(0,nDays):
		...
		accEnergy.append({'id': i, 'date': relDate, 'groupWastedEnergy': 0, 'userWastedEnergy': 0, 'groupEffectiveEnergy': 0, 'userEffectiveEnergy': 0, 'nGroupCoffees': 0, 'nUserCoffees': 0 , 'effectivePerCoffee': 0, 'average': 0})
	lastDate = relDate
		...
		if tempDate >= firstDate:
			if tempDate != auxDate:
				if isThereConsumption: 
					...
					accEnergy[next(index for (index, d) in enumerate(accEnergy) if d["date"] == auxDate)]['nGroupCoffees']        = nGroupCoffees
					accEnergy[next(index for (index, d) in enumerate(accEnergy) if d["date"] == auxDate)]['nUserCoffees']         = nUserCoffees
					if nGroupCoffees == 0:
						nGroupCoffees = 1
					accEnergy[next(index for (index, d) in enumerate(accEnergy) if d["date"] == auxDate)]['effectivePerCoffee'] = (eGroupEffectiveTemp+eGroupWastedTemp)/nGroupCoffees
					eEffectivePerCoffeeTotal += (eGroupEffectiveTemp+eGroupWastedTemp)/nGroupCoffees
				eGroupWastedTemp    	 = 0.0
				eGroupEffectiveTemp 	 = 0.0
				eUserWastedTemp     	 = 0.0
				eUserEffectiveTemp  	 = 0.0
	...
	for (index, d) in enumerate(accEnergy):
		accEnergy[index]['average'] = eEffectivePerCoffeeTotal/nDays
	return accEnergy
...
def get_personal_and_group_accumulated_energy(groupName, rfid, nDays):
	...
	for row in db.view('getGroupsInfo/'+groups_views[groupName], descending=True): # we do use of a couchdb's view 
		...
		if tempDate >= firstDate:
			if tempDate != auxDate:
				if isThereConsumption: 
					accWastedEnergy[next(index for (index, d) in enumerate(accWastedEnergy) if d["date"] == auxDate)]['groupWastedEnergy']    = eGroupWastedTemp
...
if __name__ == "__main__":
	print get_all_data_for_charts('LifeUD', '6900906A06', 10)	
